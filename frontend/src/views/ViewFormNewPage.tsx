...
/** New Page (within an existing report) — POSTs requestType: 'new_page' */
export default function ViewFormNewPage() {
  const { go } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const title = existingReport ? `${existingReport} — ${pageName}` : pageName || 'New Page';
    const { data, error } = await requestsApi.create({
      ...
      dueDate: due || null,
      payload: {
        existingReport, pageName, filevineId: filevine,
        impactsExistingAutomation: impact,
        pageGoal, audience, fields,
    ...
    <div className="view view-form">
      ...
      <div className="formcol">
        ...
        <form className="reqform" onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 20 }}>
            <label className="field-label">Page Name <span style={{ color: '#de350b' }}>*</span></label>
            <input className="txt" value={pageName} onChange={e => setPageName(e.target.value)} required />
          </div>
          <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <label className="field-label">Page Goal <span style={{ color: '#de350b' }}>*</span></label>
            <textarea className="txt txt-area" value={pageGoal} onChange={e => setPageGoal(e.target.value)} style={{ width: '100%' }} />
          </div>
          <div style={{ marginBottom: 20 }}>
